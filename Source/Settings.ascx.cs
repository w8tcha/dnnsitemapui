...
public partial class Settings : ModuleSettingsBase
{
                ...
                case "custom":
                    {
                        ...
                        foreach (var vocabulary in this._vocabularies)
                        {
                            this.cBlVocabularies.Items.FindByValue(vocabulary).Selected = true;
                        }
            ...
            case "custom" when this.cBFilterByTax.Checked:
                {
                    ...
                    if (!string.IsNullOrEmpty((string)this.TabModuleSettings["TaxVocabularies"]))
                    {
                        ...
                        this._vocabularies = taxVocabularies.Split(';');
    ...
    private void FillTabList()
    {
        foreach (var objTab in this.tabs)
        {
            ...
            {
                Text = objTab.Level switch
                {
                    1 => $"|->{objTab.LocalizedTabName}",
                    2 => $"|-->{objTab.LocalizedTabName}",
                    3 => $"|--->{objTab.LocalizedTabName}",
                    4 => $"|---->{objTab.LocalizedTabName}",
                    _ => $"|-{objTab.LocalizedTabName}"
                },
                Value = objTab.TabID.ToString()
            };
    ...
    private void InitializeComponent()
    {
        ...
        this.DefaultIcon.FileFilter = Globals.ImageFileTypes;
    ...
    private void SaveChanges()
    {
        ...
        switch (this.dDlTaxMode.SelectedValue)
        {
            case "custom":
                {
                    // Setting TaxMode Vocabularies
                    var vocabularies = string.Empty;

                    vocabularies = this.cBlVocabularies.Items.Cast<ListItem>().Where(item => item.Selected).Aggregate(
                        vocabularies,
                        (current, item) => $"{current}{item.Value};");

                    if (vocabularies.EndsWith(";"))
                    {
                        vocabularies = vocabularies.Remove(vocabularies.Length - 1);
                    }

                    objModules.UpdateTabModuleSetting(this.TabModuleId, "TaxVocabularies", vocabularies);
                    break;
                }
            ...
            case "terms":
                {
                    ...
                    terms = this.cBlTerms.Items.Cast<ListItem>().Where(item => item.Selected).Aggregate(
                        terms,
                        (current, item) => $"{current}{item.Value};");
                    ...
                    // Setting TaxMode Terms
                    objModules.UpdateTabModuleSetting(this.TabModuleId, "TaxTerms", terms);
                    break;
                }
        }
    }
