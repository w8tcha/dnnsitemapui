    ...
    public class SiteMapController : ModuleSettingsBase, IPortable, ISearchable
    {
        ...
        string IPortable.ExportModule(int iModuleId)
        {
            try
            {
                ...
                {
                    sFilterByTax = (string)this.TabModuleSettings["FilterByTax"];
                }

                if (!string.IsNullOrEmpty((string)this.TabModuleSettings["TaxMode"]))
                {
                    sTaxMode = (string)this.TabModuleSettings["TaxMode"];
                }

                if (!string.IsNullOrEmpty((string)this.TabModuleSettings["TaxVocabularies"]))
                {
                    sVocabularies = (string)this.TabModuleSettings["TaxVocabularies"];
                }
